...
class Polycube:
    def __init__(self):
        ...
        try:
            resp_req = get_req(self.endpoint, timeout=Arg_Reader.db.polycube_timeout)
    ...
    def get(self, cube):
        ...
        try:
            resp_req = get_req(f'{self.endpoint}/dynmon/{cube}', timeout=self.timeout)
            self.__manager(resp_req)
        ...
        if self.get(cube) is None:
            ...
            try:
                resp_req = put_req(f'{self.endpoint}/dynmon/{cube}',
                                   json={'dataplane-config': self.__dataplane_config(cube, code, metrics)}, timeout=self.timeout)
        ...
        if service is not None:
            ...
            try:
                ...
                resp_req = put_req(f'{self.endpoint}/dynmon/{cube}/dataplane-config',
                                   json=self.__dataplane_config(cube, code, metrics), timeout=self.timeout)
                self.__manager(resp_req)
    ...
    def __dataplane_config(cube, code, metrics):
        return {
            'ingress-path': {'name': cube, 'code': code, 'metric-configs': metrics},
            'egress-path': {}
        }
