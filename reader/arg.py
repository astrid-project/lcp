...
class ArgReader:
    ...
    def read(cls):
        ...
        add('--host', '-o', type=str,
            help='Hostname/IP of the REST Server', default=cr.lcp_host)
        add('--port', '-p', type=int,
            help='TCP Port of the REST Server', default=cr.lcp_port)

        add('--auth-max-ttl', '-t', type=str,
            help='Max authentication db TTL', default=cr.auth_max_ttl)

        add('--polycube-host', '-s', type=str,
            help='Hostname/IP of Polycube', default=cr.polycube_host)
        add('--polycube-port', '-c', type=int,
            help='Port of Polycube', default=cr.polycube_port)
