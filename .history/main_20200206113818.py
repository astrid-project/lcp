...
lcp_port = config_parser.get('lcp', 'port')

cb_endpoint = config_parser.get('cb', 'endpoint')
...
parser.add_argument('--write-config', '-w', help='Write options to config.ini',
                    action='store_true')
parser.add_('--version', '-v', help='Show version',
                    action='store_const', const=version)

args = parser.parse_args()

if arg.write_config:
    config_parser.set('lcp', 'port', args.port)
    config_parser.set('cb', 'endpoint', args.cb_endpoint)
...
if args.version is not None:
    ...
    for RouteResource in Status, Code, Config:
        for route in wrap(RouteResource.route):
            api.add_route(route, RouteResource(config_parser))
