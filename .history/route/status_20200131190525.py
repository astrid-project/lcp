...
class Status(object):
    def __init__(self, cb_host, cb_port):
        ...
        print(reg_resp)
        if reg_resp.status_code == falcon.HTTP_200 or reg_resp.status_code == falcon.HTTP_201:
            resp_data = response.json()
            self.data['alive'] = resp_data['when']
            self.data['registered'] = True
        print(f'Execute Environment ID: {self.data["id"]} correctly registered at {self.data["alive"]}')
